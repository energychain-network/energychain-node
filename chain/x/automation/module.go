...
// EndBlock runs the automation cron and stream finalization. An error here is
// fatal (consensus-level), so the keeper returns errors only for true
// invariant breaks; per-action failures are caught inside the EndBlocker.
