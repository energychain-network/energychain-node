...
// EndBlock sweeps expired view-key grants and flips them to
// revoked. Failures are logged but never abort the block —
// the sweep is bounded by params.MaxGrantsPerBlockSweep and
// the index is idempotent across blocks, so a transient
// error is retried on the next height.
func (am AppModule) EndBlock(goCtx context.Context) error {
	ctx := sdk.UnwrapSDKContext(goCtx)
	if err := am.keeper.EndBlock(goCtx); err != nil {
		ctx.Logger().Error("mrv: end-block sweep", "err", err)
	}
	return nil
}
