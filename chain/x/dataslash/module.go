...
// EndBlock runs the jail-expiry and unbond-cooldown sweeps.
// Errors are logged but the block never aborts; sweeps are
// idempotent and resume cleanly next block.
func (am AppModule) EndBlock(goCtx context.Context) error {
	ctx := sdk.UnwrapSDKContext(goCtx)
	if err := am.keeper.EndBlock(goCtx); err != nil {
		ctx.Logger().Error("dataslash: end-block sweep", "err", err)
	}
	return nil
}
