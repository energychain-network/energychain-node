...
// collectNonTransientKeys returns all KV and Object store keys (i.e. all
// store keys EXCEPT transient stores). The result is fed into BlockSTM's
// parallel-execution conflict detector. Transient stores are intentionally
// excluded because their state is wiped at every block boundary, making
// per-tx conflict tracking on them both unnecessary and incorrect.
func collectNonTransientKeys(
	keys map[string]*storetypes.KVStoreKey,
	oKeys map[string]*storetypes.ObjectStoreKey,
) []storetypes.StoreKey {
	out := make([]storetypes.StoreKey, 0, len(keys)+len(oKeys))
	for _, k := range keys {
		out = append(out, k)
	}
	for _, k := range oKeys {
...
func NewEnergyChainApp(
	...
	keys := storetypes.NewKVStoreKeys(
		...
		oracletypes.StoreKey, audittypes.StoreKey, metertypes.StoreKey,
	)
	...
	bApp.SetBlockSTMTxRunner(txnrunner.NewSTMRunner(
		...
		true,
		func(ms storetypes.MultiStore) string { return NativeDenom },
	))
	...
	app.promotedTxBroadcaster = newPromotedTxBroadcaster(logger)
	...
	*/
	...
	// set default EVM coin info as fallback for CheckTx before PreBlock runs
	app.EVMKeeper.WithDefaultEvmCoinInfo(evmtypes.EvmCoinInfo{
		Denom:         NativeDenom,
		ExtendedDenom: NativeDenom,
		DisplayDenom:  NativeDisplayDenom,
	...
	*/
	...
	app.ModuleManager.SetOrderBeginBlockers(
		...
		oracletypes.ModuleName, audittypes.ModuleName, metertypes.ModuleName,
	)
...
func (app *EVMD) Close() error {
	...
	if app.promotedTxBroadcaster != nil {
		err = errors.Join(err, app.promotedTxBroadcaster.Close())
	}
